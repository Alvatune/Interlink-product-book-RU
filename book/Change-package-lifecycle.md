<!-- Generated from wiki/Change-package-lifecycle.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Принимающий продукт может объединить публикацию данных и подготовленный снимок в одну локальную операцию «всё либо ничего». До окончательного подтверждения оба результата считаются подготовленными, а не официальными. Внешняя передача при этом не становится частью той же локальной гарантии.
...
Связанные документы: [рабочие данные](Change-package-visibility.md), [виды изменений](Change-package-variants.md), [предпросмотр и одобрение](Change-package-preview.md), [проведение и восстановление](Change-package-commit.md), [сквозные примеры](Change-package-examples.md).
