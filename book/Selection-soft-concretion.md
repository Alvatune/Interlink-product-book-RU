<!-- Generated from wiki/Selection-soft-concretion.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Сценарий не ограничивается ручным указанием версии. Необходимы выбор показанной версии, массовые команды, автоматическая конкретизация новых связей, снятие и восстановление по жизненному циклу. Interlink должен обеспечить весь этот набор результатов, а не заменить его временным параметром пакета.

**Состояние реализации:** ниже зафиксирован целевой порядок работы на 6 сентября 2026 года. Полные команды, хранение, совместная работа и интерфейс ещё требуют реализации и приёмки.
...
Для команды «Использовать показанную версию» принято условие: исходная связь ещё не имеет конкретизации. Уже имеющаяся конкретизация не исчезает от того, что её заслонил контекст или режим просмотра. Для замены существующего предпочтения нужна явная команда выбора, а не обход этого условия.
...
Процесс может предусматривать команду выключения автоконкретизации со снятием мягких предпочтений заданного уровня, в том числе ручных. Пользователь должен видеть последствия до подтверждения. Действие «не запоминать версии новых связей, но оставить существующие решения» имеет другой результат и должно быть отдельной явно названной командой.
...
Если контекст явно назначит всему объекту уплотнения Г, производственный профиль подбора может показать Г в обеих позициях. Рабочее место объясняет, что контекст заслонил локальный выбор Б, а не удалил его. Отключение этого контекста возвращает участие Б.
...
## Приёмка и вопросы специалисту предприятия
...
Далее: [жёсткая привязка](Selection-exact-fixation.md), [контексты](Selection-contexts.md), [временная проработка](Selection-temporary-preference.md), [итерации](Selection-iterations.md), [допустимые замены](Allowed-replacements.md), [путеводитель](Version-selection-guide.md).
