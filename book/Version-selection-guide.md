...
Затем полезно прочитать три страницы, которых не хватало прежнему объяснению: [полная мягкая конкретизация](Selection-soft-concretion.md), [контексты и их группы](Selection-contexts.md), [итерации и восстановление](Selection-iterations.md).
...
Внутри живого подбора производственный профиль подбора ставит применяемость перед контекстом и активной мягкой конкретизацией. Редакторский профиль Interlink может явно отдавать контексту приоритет. Оба используют один механизм выбора версии и затем её содержания. Жёсткие ограничения и требуемая точность проверяются до результата; ошибка не означает разрешение перейти к более удобному правилу.
