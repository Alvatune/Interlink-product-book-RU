<!-- Generated from wiki/Selection-iterations.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Итерация должна объединять несколько версий и оставаться доступной после завершения редактирования. Поэтому её нельзя заменить кнопкой отмены последних действий или временной точкой отката внутри пакета.

Interlink предусматривает долговечную именованную итерацию с точным сохранённым содержанием. **Статус на 6 сентября 2026 года — принятый целевой порядок работы; полный пользовательский цикл и перенос ещё не реализованы и не приняты испытаниями.**
...
Отдельно проверяются страховочная точка, необходимые переходы жизненного цикла, повтор после сбоя, доступ к файлам и точность переноса старых именованных итераций. Импорт незавершённой итерации не должен задним числом объявлять её опубликованной инженерной версией.
...
См. [контексты](Selection-contexts.md), [работа и публикация](Selection-change-package.md), [мягкая конкретизация](Selection-soft-concretion.md), [точный состав заказа](Selection-order-snapshot.md), [путеводитель](Version-selection-guide.md).
