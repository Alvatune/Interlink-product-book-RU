<!-- Generated from wiki/Order-boundaries.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Interlink должен поддерживать эти потребности в связанном процессе заказа, сохраняя различия между требованием заказчика, планом изготовления, точным выпущенным составом и фактическим исполнением.
...
## Какие данные должен включать заказ

Заказ должен содержать позиции, необходимые заказчику или производственному плану: конкретные комплектации изделий, отдельные изделия и комплекты запасных частей. Количество относится к позиции. Поверх конструкторского состава могут учитываться технология, материалы, трудоёмкость и данные для передачи в учётную систему.

При формировании производственного заказа должны учитываться правила подбора версий, контексты, головное изделие, серия, дата, опции, допустимые замены, аналоги и маршруты. Объяснение результата сообщает о недостающих значениях и неоднозначных решениях.

Процессы предприятий могут отличаться. В частности, связь производственной ведомости (ПВ), производственного заказа (ПЗ) и снимка нужно согласовать при обследовании. Поддерживаются три возможных варианта: ПВ готовит ПЗ; ПВ и ПЗ развиваются параллельно; ПВ формируется из подготовленного ПЗ или принятого снимка. Ни один вариант не объявляется универсальным заранее.
...
Инженерная версия заказа — принятая предприятием ступень его развития, например «Заказ насосов, версия Б». У неё есть рабочее содержание и последовательность опубликованных неизменяемых состояний. Исправляя количество или комментарий, пользователь сначала сохраняет рабочую правку. Публикация создаёт новое состояние выбранной версии; новую инженерную версию создают только тогда, когда этого требует предметное правило предприятия. Поэтому сохранение рабочей правки, публикация её результата и создание новой инженерной версии выполняются как разные операции.

Рабочая область отвечает за незавершённую работу. [Контекст подбора](Selection-contexts.md) отвечает за сохранённые выборы версий и условия их использования. Пакет изменения отвечает за совместную публикацию согласованного набора правок. Контекст заказа может жить дольше одного пакета, а пробный расчёт не обязан создавать пакет только ради просмотра.
...
Плановые партии и экземпляры могут создаваться отдельным действием формирования ПЗ по правилам производственный профиля. Их существование означает подготовку учёта, а не физическое изготовление. Ни количество строки, ни публикация снимка сами по себе не являются подтверждённой установкой компонентов.
...
| Потребность процесса | Решение Interlink |
|---|---|
...
## Вопросы для конкретного внедрения
...
- [Позиции и количества заказа](Order-lines-and-quantities.md)
- [Жизненный цикл живого заказа](Order-lifecycle.md)
- [Точный состав и публикации](Order-exact-publication.md)
