<!-- Generated from wiki/Selection-effectivity.md. Edit the source, then run tools/Build-Book.ps1. -->
...
## Что переносится из существующей системы

Для подбора должны учитываться головное изделие, серии или серийные номера, даты и аннулирование применяемости. Важны не только сами диапазоны, но и контроль пересечений, разрывов и границ внедрения извещения.

Цель Interlink — обеспечить обязательные пользовательские результаты. Один предполагаемый порядок нельзя объявлять универсальным для всех сочетаний условий. Правила каждого профиля должны быть определены и проверены на характерных приёмочных примерах.
...
Если переносимый сценарий предусматривает показ версии вне точной применяемости, это должно быть отдельным явно настроенным совместимым режимом. Результат сохраняет отметку несоответствия; обычная производственная публикация блокируется, пока допустимое предметное исключение не оформлено отдельно. Такой показ не превращает запрет в отсутствие правила и не даёт права обходить неотменяемые ограничения.

## Почему порядок определяется профилем

Для производственного профиля предусмотрен порядок, в котором применяемость проверяется раньше контекста и активной мягкой конкретизации. Для инженерной проработки может использоваться другой явно названный режим — например, сначала показать назначение рабочего контекста, чтобы оценить будущее изменение.
...
Приоритет серии перед датой должен быть явно закреплён в соответствующем профиле. Этого недостаточно для всех пересечений: например, специальная запись для головного изделия по дате может совпасть с общей записью по серии. Для таких сочетаний нужна полная согласованная таблица решений. Пока конкретный случай не определён, система сообщает неоднозначность или неподдерживаемый режим, а не выбирает порядок случайно.
...
Со специалистом предприятия нужно согласовать смысл «серии», границы диапазонов, аннулирование части области, намеренные пробелы и сочетание специальных и общих записей. Спорные комбинации следует превратить в приёмочные испытания.
...
Описан принятый целевой подход. Полные проверки диапазонов, профили совместимости, интерфейс и автоматическое разбиение заказов требуют реализации и испытаний. Неподтверждённые сочетания правил остаются открытыми до проверки на приёмочных данных.

## Связанные документы

- [Контексты подбора](Selection-contexts.md)
- [Мягкая конкретизация](Selection-soft-concretion.md)
- [Условия включения позиции](Selection-position-presence.md)
- [Точный состав заказа](Selection-order-snapshot.md)
