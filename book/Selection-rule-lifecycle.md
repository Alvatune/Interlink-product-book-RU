<!-- Generated from wiki/Selection-rule-lifecycle.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Если инженер явно удалил выбор или заменил его новым, старое процессное событие не должно восстановить прежнее решение. Операции «приостановить», «удалить» и «не учитывать в этом просмотре» имеют разный смысл. Именно это требуется проверить при переносе мягкой конкретизации.
...
Для жёсткой привязки всегда определяется требуемая точность: инженерная версия или точное содержание. В регламентированном процессе её назначает предусмотренная системная или процессная команда; обычная пользовательская конкретизация относится к мягкому выбору. Прикладной продукт на Interlink может отдельно разрешить уполномоченному специалисту ручное назначение жёсткой привязки. Это самостоятельное расширение прав, а не автоматическое право каждого конструктора.
...
## Что проверить со специалистом предприятия
...
Описан целевой процесс управления правилами и управляющими данными. Конкретные редакторы, административные рабочие места, испытательные наборы и показатели относятся к последующим этапам реализации. Порядок зависимостей замен и подбора уже определён предметно; точные профили совместимости с установками исходных систем подтверждаются сравнительными испытаниями.

## Связанные документы

- [Настраиваемые правила](Selection-custom-rule.md)
...
- [Групповые замены](Selection-group-substitution.md)
