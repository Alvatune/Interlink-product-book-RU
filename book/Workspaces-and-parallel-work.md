<!-- Generated from wiki/Workspaces-and-parallel-work.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Это позволяет отделить три решения: начать или продолжить редактирование; выбрать условия совместного просмотра; выпустить определённый результат. Для третьего решения используется [пакет изменения](Change-package-concept.md), для второго — [контекст подбора](Selection-contexts.md).
...
Различаются обычное автоматическое пополнение контекста и назначения от извещения. При включённом обычном режиме взятие на изменение, создание новой инженерной версии и нового объекта в составе обновляют предусмотренные назначения. Простое присоединение уже существующего объекта не объявляется общим поводом такого пополнения. Выключение обычного режима не отключает независимую синхронизацию извещения.
...
Производная часть контекста извещения меняется через строки извещения. Ручная правка автоматически выведенного назначения не должна рассогласовывать извещение и контекст. Конкретные события и допустимые реакции проверяются на согласованных сценариях предприятия.
...
## Что нужно проверить с продуктовым специалистом
...
Рабочие области, разделение версий и состояний, правила контекстов и конфликты описаны как целевой контракт архитектуры на 6 сентября 2026 года. Сквозная реализация и пользовательские экраны ещё требуют разработки и испытаний. Будущие агентские циклы используют эти же границы работы, но не определяют обязательный интерфейс для обычного конструктора.

Связанные страницы: [пакет изменения](Change-package-guide.md), [рабочая видимость](Change-package-visibility.md), [контексты](Selection-contexts.md), [итерации](Selection-iterations.md), [мягкая конкретизация](Selection-soft-concretion.md), [агенты и прослеживаемость](Agents-and-traceability.md).
