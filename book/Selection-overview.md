<!-- Generated from wiki/Selection-overview.md. Edit the source, then run tools/Build-Book.ps1. -->

# Как устроен подбор версий в Interlink

<a id="сохраняется-результат-уточняется-устройство"></a>

## Какую задачу решает подбор

Interlink должен поддерживать плавающий состав: позиция указывает на дочерний объект, а нужная версия определяется при чтении. Благодаря этому новая версия повторно используемого компонента не заставляет немедленно создавать новую версию каждого родительского изделия. Предусмотрены жёсткая и мягкая конкретизация, применяемость, связанные контексты, именованные правила и точная фиксация производственного результата.

Чтобы совместный просмотр не заставлял всех участников одновременно завершать работу, Interlink разделяет рабочую область, контекст подбора и пакет изменения. Мягкая конкретизация хранит долговечное предпочтение конкретной позиции; временный выбор служит отдельной проработке. Такое разделение должно обеспечить обязательные пользовательские сценарии и предсказуемую параллельную работу людей, интеграций и агентов.

Документ описывает **принятый порядок работы на 6 сентября 2026 года**, а не готовый промышленный модуль.
...
Отдельная [страница мягкой конкретизации](Selection-soft-concretion.md) раскрывает ручные, автоматические и массовые операции. [Временная проработка](Selection-temporary-preference.md) остаётся полезной, но не является её заменой.

<a id="почему-всё-ещё-один-механизм-подбора"></a>

## Что означает единый механизм подбора
...
| Источник после жёстких и строгих ограничений | Производственный профиль | Редакторский профиль Interlink |
|---|---|---|
...
Производственный профиль задаёт приоритет условий выпуска и применяемости. Для специальных диапазонов и их пересечений нужна согласованная таблица решений. Редакторский профиль не включается вместо производственного скрыто.
...
<a id="что-находится-рядом-с-подбором-но-не-растворяется-в-нём"></a>

## Какие связанные процессы выполняются отдельно от подбора версий
...
Для редуктора серии 145 применим подшипник версии Б. Конструкторский контекст назначает версию В, ещё проходящую испытания. В производственном профиле подбора подтверждённая применяемость выбирает Б. В редакторском профиле контекст может выбрать В, а разрешённый рабочий режим — её рабочую копию.
...
## Как доказать покрытие сценариев

Нужны приёмочные сценарии: две мягкие конкретизации одного объекта, выключение и восстановление по жизненному циклу, объединение контекстов и независимое завершение, создание новой версии от исторического прототипа, итерации, диапазоны применяемости, именованные правила, аналоги и полные групповые замены. Для каждого случая заранее фиксируются исходные данные, ожидаемый результат и недопустимые побочные эффекты.
...
Далее: [путеводитель по всем сценариям](Version-selection-guide.md), [контексты](Selection-contexts.md), [жёсткая и точная привязка](Selection-exact-fixation.md), [итерации](Selection-iterations.md), [совместимость](Compatibility-matrices.md).
