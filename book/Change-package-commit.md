<!-- Generated from wiki/Change-package-commit.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Ниже описан целевой порядок работы, уточнённый в сентябре 2026 года, а не уже готовое рабочее место.
...
| Исход общей фиксации неизвестен | Связь потеряна до подтверждения успеха или отката | Не начинать новое проведение под другим номером; выяснить исход прежней операции и использовать предусмотренный протокол восстановления |
...
sequenceDiagram
    ...
    А-->>П: Исход неизвестен: сначала сверка прежней операции
...
План предусматривает неделимое проведение выбранных действий, сохранение остальной работы, возможность установить результат операции после потери ответа и отдельный учёт публикации и доставки. Полные механизмы выполнения и пользовательские экраны ещё предстоит реализовать. Для приёмки нужны испытания конфликтов, обрывов связи и перезапуска, а также проверка того, что восстановление материалов в работу не становится скрытой публикацией.

Связанные документы: [рабочее состояние и параллельность](Change-package-visibility.md), [предпросмотр и согласование](Change-package-preview.md), [снимки и доказательства](Change-package-snapshots.md), [итерации и восстановление](Selection-iterations.md), [мягкая конкретизация](Selection-soft-concretion.md).
