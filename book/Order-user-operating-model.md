<!-- Generated from wiki/Order-user-operating-model.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Самостоятельный результат подготовки производственного состава. Рабочий ПЗ может содержать ещё не снятую вариантность, несколько маршрутов и предупреждения: пользователь должен иметь возможность сохранить незавершённый план и продолжить подготовку. Он становится готовым заданием после проверки выбранной области выдачи и принятия по правилам предприятия. Его процессный жизненный цикл — подготовка, запуск, исполнение, приостановка и закрытие — принадлежит принимающему продукту, обычно ERP или MES. Interlink хранит инженерное содержание и точные основания, не заменяя этот производственный процесс.
...
[Справочные таблицы](Reference-tables.md) позволяют искать сортамент по размерам и материалу, [многомерные таблицы](Multidimensional-tables.md) — читать норму или испытание по нескольким условиям, [координатные карты](Coordinate-maps.md) — получать кандидатов изделия либо покрытия. Таблица не выбирает за технолога, если результат неоднозначен. Отсутствующая точка не становится нулевой нормой. Для выпуска сохраняются точная редакция данных, использованные значения, единицы, формулы и округления.
...
Один универсальный порядок между ПВ и ПЗ не устанавливается: разные процессы могут требовать следующего:
...
## Как устроено решение Interlink

Interlink сохраняет функциональную потребность предприятия в комплектациях, производственных ведомостях, заказах, точном составе, партиях и экземплярах. При этом:
...
Точные границы ПВ, ПЗ, партий и экземпляров необходимо проверить на процессах нескольких предприятий.
...
Связанные документы: [Общая модель и границы заказного контура](Order-boundaries.md), [Жизненный цикл живого заказа](Order-lifecycle.md), [Точный состав и публикации заказа](Order-exact-publication.md), [Формирование производственного заказа](Order-production-composition.md), [Экземпляры, партии и фактический состав](Order-units-lots-as-built.md), [MRP, ERP/MES и длительные операции](Order-integration.md).
