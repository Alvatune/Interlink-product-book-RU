<!-- Generated from wiki/Order-roles-and-approval.md. Edit the source, then run tools/Build-Book.ps1. -->
...
```mermaid
flowchart LR
    A["Автор заказа<br/>позиции и количества"] --> B["Специалист по комплектации<br/>опции и полнота"]
    B --> C["Конструктор<br/>версии изделия и документы"]
    C --> D["Технолог<br/>маршруты и нормы"]
    D --> E["Планировщик<br/>обеспечение и площадка"]
    E --> F["Согласующие<br/>разрешение выпуска"]
    F --> G["Доверенная публикация<br/>полный снимок"]
    G --> H["Интегратор<br/>контролируемая передача"]
```
...
Если часть работы выполняет ИИ-агент, он не получает безымянные права всей системы. Сохраняются его собственная личность, поручивший работу человек или другой агент, область разрешённых действий и конкретно показанные ему данные. Агент может предложить замену или подготовить расчёт; право утвердить и применить результат определяется отдельно. После сбоя продолжение работы не должно повторно выдать то же задание под новым номером.

## Согласование конкретного содержания

Подпись должна относиться к точному содержанию данных. Обозначения инженерной версии недостаточно, если в ней может появиться следующее опубликованное состояние. Нельзя подписать «заказ вообще», а затем незаметно заменить содержание чертежа или маршрут. В записи согласования указываются:
...
Выпуск новой редакции нормы сам по себе не переписывает предмет прежней подписи. Если согласовано конкретное содержание, оно остаётся исторически согласованным. Но перед новым применением отдельно проверяется сегодняшний допуск: отзыв квалификации, запрет материала или прекращение полномочий могут запретить новую выдачу. Историческое доказательство и действующее разрешение — разные вопросы.
...
2. Доверенная служба имеет самостоятельное право сформировать полный снимок, а интерфейс показывает пользователю только разрешённое представление, не изменяя самого снимка. Сам факт запуска службы не даёт пользователю её полномочий и не раскрывает закрытые данные через пояснения.
...
Технолог не утверждает замену единолично. После согласования публикуется новое содержание конструкции, норм и затронутого заказа. Требует ли изменение материала новых инженерных версий, задаёт предметный регламент; одного сохранения правки для выпуска недостаточно. Старые подписи остаются в истории предыдущей передачи.
...
Автор ПВ исправляет ошибочную ссылку на инструкцию в рабочем содержании ведомости. Прежняя подпись не распространяется на исправленное содержание. Согласующие видят точную разницу и подписывают новый опубликованный результат; новая инженерная версия ПВ создаётся только по регламенту. Уже созданный производственный снимок не редактируется. Если меняется передаваемый состав или обязательный комплект его оснований, оформляется новая публикация; иначе исправленная ведомость может ссылаться на прежнее точное основание.

## Что именно разрешает согласование замены

Разрешение [альтернативного комплекта](Allowed-replacements.md) подтверждает его пригодность в указанной области. Заказ отдельно выбирает этот комплект для своих позиций. Перед выдачей проверяются полные участники, количества, сохраняемые узлы и [совместимость выбранного содержания](Compatibility-matrices.md). Подпись не превращает недостающую обязательную деталь в необязательную и не отменяет неотменяемое требование безопасности.

Технолог, например, может принять покрытие из [справочника](Reference-tables.md) для стального корпуса и морской среды. Для алюминиевого корпуса тот же выбор требует своего основания. Проверка сохраняет конкретную редакцию нормы, условия и использованные испытания. Изменение справочника не переписывает старое решение, но его отзыв может потребовать остановить новую передачу и проанализировать затронутые заказы.

Даже после выдачи задания заменённый двигатель ещё не считается установленным. Фактический состав меняется только по принятому подтверждению установки конкретного экземпляра. Разрешение, отгрузка детали со склада и физический монтаж — три разных события.
...
Обязательные записи об авторе, результате операции и задании отправки сохраняются вместе с локальным предметным эффектом. Нельзя оставить опубликованный состав без обязательного подтверждения только потому, что приложение оборвалось сразу после сохранения. Потерянный ответ не считается откатом: система устанавливает действительный исход и не создаёт вторую выдачу наугад. Срок хранения определяется назначением, нормативными обязательствами и запретами удаления; слово «неизменяемый» не означает автоматического вечного хранения всех копий.

## Открытые вопросы

- Какие роли и этапы необходимы в выбранном процессе предприятия?
...
- [Точный состав передачи](Order-exact-publication.md)
- [Производственные ведомости](Order-production-documents.md)
- [Доставка модели и границы принимающей системы](Extensibility-and-migration.md)
