<!-- Generated from wiki/Data-type-system-vision.md. Edit the source, then run tools/Build-Book.ps1. -->
...
При проектировании нужно совместить возможность добавлять новые признаки с предсказуемым хранением и проверкой данных. Interlink предусматривает два согласованных способа развития: основная предметная структура компилируется в типизированную схему, а редкие дополнительные признаки хранятся как управляемые расширения. Для часто используемого расширения предусмотрен переход в основную схему. Это выбранный компромисс, а не утверждение, что любые другие архитектуры обязательно медленнее или менее гибки.

```mermaid
flowchart TD
    A["Базовая модель поставщика"] --> D["Итоговая модель предприятия"]
...
Определения данных в Interlink сводятся в одну проверяемую модель. Прикладные процессы, согласование и отраслевые правила используют её, но не выводятся автоматически из перечня полей: им нужны явно заданные правила и собственная приёмка.
...
Такой выбор формы записи позволяет использовать Interlink в логистике, управлении качеством и других областях без искусственных конструкторских процедур. Подробнее — [объектный фундамент](Object-foundation.md).
...
## Какие принципы объединяет система типов

- предметная глубина: объекты, инженерные версии и самостоятельные позиции состава;
- физическая определённость: известные типы материализуются в типизированные таблицы;
- проверяемая поставка: описание модели порождает согласованные артефакты;
- управляемые расширения: настройки предприятия имеют объявленные границы;
- контролируемая эволюция: миграции проверяются, применяются по плану и оставляют журнал.

Эти принципы должны работать совместно: одна предметная модель задаёт смысл данных, типизированных запросов и команд. Сами по себе таблицы или генерация кода ещё не доказывают правильность пользовательского процесса.
...
- [Предметные возможности системы типов](Data-type-system-capabilities.md)
- [Физическая материализация](Data-database-materialization.md)
- [Расширяемость](Data-extensibility.md)
