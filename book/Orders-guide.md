<!-- Generated from wiki/Orders-guide.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Раздел предназначен для продуктовых специалистов и экспертов предприятий, специалистов по конфигурации изделий, планировщиков производства, технологов и руководителей внедрения систем управления жизненным циклом изделия (PLM). Он объясняет работу с заказами — от потребности заказчика до точной передачи состава в производство и последующей связи с партиями, экземплярами и внешними учётными системами.

Главный вопрос раздела:

> Как связать живое планирование, точный переданный состав, производственные документы и фактическое исполнение так, чтобы каждое состояние было воспроизводимо и объяснимо?

Документы описывают принятые требования и будущие действия пользователей, а не готовый модуль Interlink. Основные сценарии работы с заказами должны проверяться уже на опытном фундаменте: например, сохранение производственного заказа с нерешёнными вариантами, передача части количества и замена целого комплекта компонентов. Так ограничения подбора и хранения можно обнаружить до создания полноценных рабочих мест и производственных интеграций.
...
Рабочий производственный заказ (ПЗ) может быть сформирован ещё до окончательного выбора заменителей и маршрутов. Специалисты сохраняют его, видят нерешённые места и продолжают подготовку. Поддержка такого сценария обязательна; сохранение рабочего заказа не означает, что производство уже приняло задание.
...
Такой подход сохраняет важное ожидаемое поведение: заказ можно развивать, но ранее переданный производственный результат остаётся неподвижным и доступным для разбора истории.
...
## Обязательные сценарии работы с заказом

Interlink должен обеспечить следующие потребности:
...
Заказ может выражать сбытовую потребность, но это не означает, что инженерный модуль должен владеть ценами, сроками, складами и коммерческим портфелем. Эти сведения могут оставаться в ERP или другом приложении. Для внедрения определяют владельца каждого вида данных и необходимые связи: такое разделение не должно приводить к потере нужного пользователю сквозного процесса.
...
Продолжение: [Допустимые замены](Allowed-replacements.md), [Матрицы совместимости](Compatibility-matrices.md), [Справочные таблицы](Reference-tables.md), [Многомерные таблицы](Multidimensional-tables.md), [Координатные карты подбора](Coordinate-maps.md).
...
- [Общая модель и границы заказного контура](Order-boundaries.md)
- [Позиции заказа, количества и несколько изделий](Order-lines-and-quantities.md)
- [Комплектации, опции и точное изделие](Order-configuration.md)
- [Жизненный цикл живого заказа](Order-lifecycle.md)
- [Точный состав и публикации заказа](Order-exact-publication.md)

### Подготовка производства

- [Формирование производственного заказа](Order-production-composition.md)
- [Маршруты, нормы и решения о способе обеспечения](Order-routes-and-norms.md)
- [Производственные ведомости, документы и отчёты](Order-production-documents.md)

### Исполнение и изменения

- [Экземпляры, партии и фактический состав](Order-units-lots-as-built.md)
- [Перепланирование, замены и история изменений](Order-replanning.md)
- [MRP, ERP/MES и длительные операции](Order-integration.md)
- [Роли, согласование и права](Order-roles-and-approval.md)

### Сквозная проверка

- [Сквозные машиностроительные сценарии](Order-end-to-end.md)
- [Покрытие, открытые вопросы и состояние реализации](Order-coverage-and-status.md)
- [Как пользователи ведут заказ от потребности до производства](Order-user-operating-model.md)

<a id="как-читать-пакет-на-продуктовой-встрече"></a>

## Как обсуждать заказы на продуктовой встрече

Рекомендуемый порядок:

1. Сначала пройти [полный пользовательский путь](Order-user-operating-model.md) и согласовать различие живого заказа, точного снимка, производственной ведомости, производственного заказа и фактического изделия.
...
7. Зафиксировать ответы в профиле внедрения и приёмочных сценариях.
...
Таким образом, документы раздела описывают требования, принятые решения и основу будущей приёмки. Они не являются подтверждением готовности пользовательского модуля заказов.

## Основные источники

- [Пользовательские сценарии и решения](User-scenarios-and-solutions.md)
- [Формирование состава в Interlink](Versions-and-composition.md)
- [Версии, изменения и снимки](Changes-and-evidence.md)
- [Карта обязательных сценариев](Scenario-coverage.md)
- [Состояние и дорожная карта](Current-state-and-roadmap.md)
- [Глоссарий](Glossary.md)
- [Источники, статусы и доверие](Trust-and-sources.md)

Публичная книга передаёт выводы из рабочих материалов своими словами. Принципы проверки и ограничения публикации описаны в [разделе об источниках и доверии](Trust-and-sources.md).
