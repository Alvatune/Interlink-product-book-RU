<!-- Generated from wiki/Data-extensibility.md. Edit the source, then run tools/Build-Book.ps1. -->
...
```mermaid
flowchart TD
    A["Защищённая от произвольных правок<br/>основная модель"] --> D["Итоговая модель предприятия"]
...
|---|---|---|---|
| Богатый конфигуратор предприятия, знакомый специалистам предприятий | Быстрое отражение местных понятий и правил | Зависимости, перенос между средами и воспроизводимость | Поставляемая модель и явно разрешённые изменения предприятия |
...
Таблица сравнивает архитектурные приёмы, а не ранжирует продукты. Разные PLM сочетают их по-разному; наличие конфигуратора само по себе не означает отсутствие управляемой поставки. Interlink должен доказать удобство и обновляемость выбранного сочетания на реальных сценариях.
...
Когда таблица используется как основание расчёта или выпуска, отдельно фиксируется необходимое точное содержание. Изменение оси или добавление значения не расширяет старую редакцию задним числом. Подробнее — [табличные данные](Tabular-data.md).
...
- [DSL и модули](Data-model-language.md)
- [Материализация в БД](Data-database-materialization.md)
- [Выполнение миграций](Data-migration-execution.md)
