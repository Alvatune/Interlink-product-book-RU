<!-- Generated from wiki/Data-status-risks-acceptance.md. Edit the source, then run tools/Build-Book.ps1. -->
...
| Допустимые замены и совместимость | Приняты предметные договоры поверх общей модели | Выбор в контексте, групповые замены, окончательная проверка содержания и проверка согласованных сценариев |
| Перенос метаданных и данных исходной системы | Стратегия описана | Средства извлечения, черновик модели, пробные прогоны и сверка |
...
Предметные таблицы, точные индексы и предсказуемые запросы должны уменьшить издержки универсального хранения. Поэтому архитектура Interlink рассчитана на интенсивные чтения, обходы состава и параллельные действия агентов. Допустимый масштаб и устойчивость под нагрузкой предстоит подтвердить измерениями.

До завершения измерений нельзя обещать конкретное число одновременных агентов, фиксированное превосходство во всех запросах или гарантированное ускорение любой базы исходной системы. Итог зависит от объёма состава, распределения признаков, оборудования, настроек PostgreSQL, характера прав доступа и профиля операций.

### Что пока нельзя объявлять готовым

- автоматический перенос произвольной базы исходной системы без обследования;
...
|---|---|---|
| Предметная модель слишком рано закрепила неверные границы типов | Частые тяжёлые изменения таблиц | Опытный срез, анализ реальных данных предприятия, устойчивые идентификаторы и дисциплина изменений |
...
| При переносе потеряется скрытая семантика исходных данных | Формально полные, но неверно интерпретированные данные | Инвентаризация форм, обработчиков и практик; участие продуктовых специалистов; контрольные машиностроительные сценарии |
...
Для каждой операции фиксируются обычное и замедленное время ответа, потребление памяти, план выполнения, число затронутых строк и поведение при росте параллельности. Сформированные системой структурные запросы сравниваются с подготовленными вручную запросами, возвращающими тот же результат с теми же правилами подбора и доступа. В плане опытного подтверждения заложен ориентир — не хуже чем в полтора раза для закреплённых структурных сценариев. Это критерий к исполнителю Interlink, а не обещание ускорения любой системы.

## Программа подтверждения переноса существующих данных
...
Для воспроизводимого испытания нагрузки нужен прогноз использования Alvatune: число активных агентов, частота чтений и изменений, типичная глубина состава, размер контекстного снимка и доля параллельных операций над одним объектом.
...
- контрольный набор существующих данных перенесён с доказанным сохранением смысла;
...
До этого момента корректная формулировка такова: **Interlink спроектирован для снижения конкретных издержек хранения и работы при высокой параллельной нагрузке. Допустимые объёмы, время ответа и границы применимости ещё предстоит подтвердить измерениями.**
...
- [Физическая структура базы](Data-database-materialization.md)
- [Производительность и агентская нагрузка](Data-performance-and-agents.md)
- [Выполнение и доказательство переноса](Data-migration-execution.md)
