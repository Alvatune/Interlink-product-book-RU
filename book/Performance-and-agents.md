<!-- Generated from wiki/Performance-and-agents.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Здесь описан **целевой подход и программа его проверки**. Компоненты типизированного хранения уже разрабатываются в составе фундамента. Полный агентский режим и целевое быстродействие ещё не подтверждены сквозными нагрузочными испытаниями.
...
Такой же подход нужен для индексируемых справочников и многомерных данных: система должна получать нужную область значений, а не обязательно выгружать всю таблицу как файл. Общая картина — в [путеводителе по данным](Data-guide.md).
...
## Предметные требования и масштабирование
...
Планируемый способ достижения масштаба включает типизированное физическое хранение, определённые формы запросов, пакетную обработку, повторно читаемые точные комплекты и управление машинной нагрузкой. Ожидаемый эффект — **архитектурная гипотеза**, которую необходимо проверить с учётом индексов, настроек, прав доступа и состава модулей.
...
В проекте существуют компоненты описания модели и материализации типизированного хранения. Единые гарантии чтения, действий, восстановления и смешанной нагрузки зафиксированы как требования к целевому поведению и требуют последовательной реализации и испытаний. Эта страница не заявляет уже достигнутых промышленных показателей.

Связанные объяснения: [система типов и данные](Data-guide.md), [объектный фундамент](Object-foundation.md), [агенты и прослеживаемость](Agents-and-traceability.md), [безопасные действия агентов](Agent-safe-actions.md), [текущая готовность и план](Current-state-and-roadmap.md). Для обсуждения нагрузки предприятия — [вопросы об агентах и испытаниях](Questions-agents.md).
