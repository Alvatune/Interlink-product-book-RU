<!-- Generated from wiki/Change-package-visibility.md. Edit the source, then run tools/Build-Book.ps1. -->

# Рабочие данные, видимость и параллельность

<a id="главное-изменение-работа-больше-не-заперта-внутри-одного-пакета"></a>

## Одна рабочая область — несколько последовательных выпусков
...
Это целевая модель, а не описание уже готовых экранов. Она предусматривает исключительное взятие версии на изменение для процессов, которым это необходимо, но не делает такое ограничение единственным способом работы всей системы.
...
Предприятие может разрешать изменение выбранной инженерной версии только одной работе. Другой пользователь получает объяснение занятости в пределах своих прав. Это удобно для регламентированных процессов и для данных, которые трудно согласовывать параллельно.
...
Отдельно существуют временное предпочтение для проработки и **мягкая конкретизация позиции** — сохраняемое предметное предпочтение с собственными условиями действия. Нельзя требовать удалить мягкую конкретизацию только потому, что закончился пакет. И нельзя автоматически превратить временный выбор двигателя для всей проработки в постоянные решения каждой позиции. Подробнее — [мягкая конкретизация](Selection-soft-concretion.md).
...
Связанные документы: [рабочие области](Workspaces-and-parallel-work.md), [контексты подбора](Selection-contexts.md), [итерации](Selection-iterations.md), [проведение и восстановление](Change-package-commit.md), [агенты и прослеживаемость](Agents-and-traceability.md).
