<!-- Generated from wiki/Change-package-variants.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Не каждая запись требует инженерной версии. **Пакет изменения нужен там, где выбранный набор действий должен быть подготовлен и проведён совместно.** Прикладное приложение может скрыть подготовку маленького пакета за одной понятной кнопкой, но не должно создавать фиктивные версии у неверсионных данных.
...
Назначение основной версии двигателя — самостоятельное управляемое решение. Меняется правило обычного выбора версии; геометрия и содержание самих версий двигателя при этом остаются прежними. Система проверяет допустимость назначения в нужной области и сохраняет, кто изменил обычный выбор.
...
Перенос строк извещения из существующей системы должен сохранять предметный смысл и происхождение назначений. Неясные правила конкретной установки проверяются по её примерам; автоматическое удаление всех «похожих» назначений недопустимо.
...
Подробнее: [справочные таблицы](Reference-tables.md), [полные многомерные таблицы](Multidimensional-tables.md) и [разреженные координатные карты](Coordinate-maps.md).
...
Связанные документы: [границы пакета](Change-package-concept.md), [путь пользователя](Change-package-lifecycle.md), [рабочие области](Workspaces-and-parallel-work.md), [допустимые замены](Allowed-replacements.md), [матрицы совместимости](Compatibility-matrices.md), [сквозные примеры](Change-package-examples.md).
