<!-- Generated from wiki/Order-lifecycle.md. Edit the source, then run tools/Build-Book.ps1. -->
...
[Контекст подбора](Selection-contexts.md) отдельно задаёт, какие версии предпочитать при проверке будущего результата. Он может использоваться несколькими рабочими областями и переживать отдельный пакет. Постоянное мягкое предпочтение позиции также не исчезает только потому, что пакет завершён.
...
- Какие состояния заказа обязательны для выбранного процесса предприятия?
