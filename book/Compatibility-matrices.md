<!-- Generated from wiki/Compatibility-matrices.md. Edit the source, then run tools/Build-Book.ps1. -->
...
В Interlink совместимость планируется как явный набор предметных ограничений с известными основаниями и областью проверки. Матрица — один из удобных способов показать и редактировать эти ограничения. Наличие записи в матрице имеет смысл только вместе с заданным правилом: это может быть разрешение, запрет или ещё не проверенное сочетание. Перед применением система должна учесть область действия записи и остальные обязательные ограничения.

**Статус: принятый целевой порядок работы и план реализации.** Проверка простых отношений, обязательные сценарии и дальнейший сложный поиск решений имеют разные этапы. Наличие схемы хранения или текста правила ещё не доказывает, что его исполнитель уже готов.
...
Обычная [карта сопоставлений](Coordinate-maps.md) не считает пустую клетку ни разрешением, ни запретом. Более сильный вывод возможен только тогда, когда объявлен полный перечень для известной области и проверено всё его авторитетное содержание.
...
Список только известных запретов не обязательно является полным. Нельзя заключить «остальное можно» по неполной странице, устаревшей копии, скрытым данным или прерванной проверке. Полная сетка хранения также не гарантирует полноту знаний: для клетки может быть указан статус «не исследовано».
...
Будущий исполнитель может уметь проверить уже полный комплект, но не уметь искать все способы его достроить. Это допустимая граница возможностей, если пользователь видит, что проверка готового комплекта поддерживается, а поиск допустимых продолжений ещё нет. Отсутствие такой функции не означает, что частичный выбор технически невозможен.
...
## Какие требования покрывает Interlink и что добавляется
...
При переносе существующих матриц нельзя терять смысл «и / или», пустот и порядка там, где он значим. Отдельно проверяются различия исходных правил и их редакций. Использование членства материала в группах планируется как явно настраиваемая возможность; включать его без проверки согласованного смысла матрицы нельзя.
...
Связанные материалы: [карты сопоставлений](Coordinate-maps.md), [полные сетки](Multidimensional-tables.md), [допустимые замены](Allowed-replacements.md), [условия присутствия позиций](Selection-position-presence.md), [заказное конфигурирование](Order-configuration.md).
