<!-- Generated from wiki/Data-migration-execution.md. Edit the source, then run tools/Build-Book.ps1. -->
...
1. **Подготовку модели по описаниям исходной системы** — получение черновика типов, свойств и связей для проверки специалистами. Сами изделия и заказы при этом ещё не перенесены.
2. **Первичный переход с существующих систем** — перенос предметных данных и проверку пользовательского поведения между разными архитектурами.
...
```mermaid
flowchart TD
    A["Именованный согласованный исходный срез исходной системы"] --> B["Проверка и карантин дефектов"]
...
- сохранность глобальных идентификаторов исходной системе и таблицы соответствий;
...
Простое совпадение числа строк недостаточно: одна универсальная строка исходной системы может разложиться на несколько предметных строк Interlink или, наоборот, несколько технических записей объединиться в один объект.

## Функциональная сверка

Одни и те же пользовательские сценарии исполняются в исходной системе и целевом контуре:
...
Известное расхождение исторических источников исходной системе по порядку применяемости и конкретизации нельзя закрыть предположением. Оно проверяется на целевой установке и фиксируется в профиле миграции.
...
Результат включает время ожидания пользователя, число обслуживаемых операций, поведение при пиках и потребление ресурсов. Техническая команда прикладывает подробные измерения. Только такой протокол позволяет обоснованно сказать, где Interlink быстрее исходной системы и какова эксплуатационная цена новой модели.
...
По документированному срезу сентября 2026 года уже существуют проверка модели, смысловое сравнение, классификация риска и планировщик, формирующий описание перехода без применения к действующей базе. Есть основа каталога модели; создание предметных таблиц развивается отдельно. Полный исполнитель установки, безопасная активация, промышленный загрузчик исходной системе и инструментарий сравнительной приёмки пока не образуют готового сквозного механизма. Описанный процесс — целевой регламент, по которому предстоит доказать готовность первой миграции.

## Связанные документы

- [Стратегия миграции существующих данных](Data-migration-strategy.md)
- [Предметный язык и редакции модели](Data-model-language.md)
- [Выпуск и установка проверенной модели](Data-release-and-installation.md)
- [Статусы и риски](Data-status-risks-acceptance.md)
- [Многомерные таблицы](Multidimensional-tables.md)
- [Соответствия по нескольким осям](Coordinate-maps.md)
