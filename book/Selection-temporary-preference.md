<!-- Generated from wiki/Selection-temporary-preference.md. Edit the source, then run tools/Build-Book.ps1. -->

# Временное предпочтение для проработки — не вся мягкая конкретизация

## Что исправлено в понимании механизма

Мягкая конкретизация сохраняет пользовательский выбор, может временно переставать действовать на определённом шаге жизненного цикла и восстанавливаться позднее. Временное предпочтение служит ограниченной проработке. Различия в сроке действия и правилах завершения не позволяют объединить их в одну настройку пакета.

Поэтому в Interlink предусмотрена отдельная [полная мягкая конкретизация](Selection-soft-concretion.md). Эта страница описывает более узкую возможность: временно предпочесть инженерную версию для испытания варианта, не объявляя это постоянным решением.
...
Взаимодействие временного выбора, долговечного контекста, мягкой конкретизации и применяемости зависит от явно выбранного профиля. Редакторский режим может ставить контекст перед применяемостью, а производственный — отдавать приоритет условиям применения.
...
Если вариант отклонён, предпочтение прекращается. Если он нужен с серии 201, оформляется применяемость. Если его требуется сохранить на определённой позиции для дальнейшей инженерной работы, создаётся соответствующая мягкая конкретизация. История должна различать первоначальное временное предпочтение и последующее решение: отклонить его, задать применяемость или сохранить мягкую конкретизацию.
...
Поэтому используется явно ограниченное решение конфигурации для пути левой установки. Оно не меняет локальный состав типовой подсборки во всех её применениях. Такое адресное расширение Interlink полезно само по себе, но не должно выдаваться за доказанный внутренний способ хранения мягкой конкретизации.
...
## Приёмка и вопросы к специалистам
...
- [Полная мягкая конкретизация](Selection-soft-concretion.md)
- [Контексты и совместная работа](Selection-contexts.md)
- [Жёсткие и точные привязки](Selection-exact-fixation.md)
- [Применяемость](Selection-effectivity.md)
