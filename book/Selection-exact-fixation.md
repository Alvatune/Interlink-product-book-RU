<!-- Generated from wiki/Selection-exact-fixation.md. Edit the source, then run tools/Build-Book.ps1. -->

# Жёсткая привязка к инженерной версии и фиксация точного содержания

<a id="почему-теперь-нужны-два-понятия"></a>

## Два разных требования к привязке
...
В первом случае запрещена подмена инженерной версии, но в разрешённом процессе у самой версии 05 может появиться исправленное содержание. Во втором нельзя подменить даже содержание **той же версии**. В Interlink оба случая предусмотрены явно: закрепление версии и закрепление точного содержания решают разные пользовательские задачи.

## Что должна гарантировать жёсткая конкретизация

Жёсткая конкретизация закрепляет выбранную дочернюю инженерную версию и не позволяет обычному правилу заменить её последней или базовой. Это самостоятельное обязательное требование к Interlink.
...
Способ назначения зависит от процесса. В регламентированном режиме жёсткую привязку устанавливает предусмотренная системная или процессная команда; обычный разрешённый пользовательский выбор версии относится к мягкой конкретизации. Свободное ручное назначение жёстких ссылок не должно появляться в таком процессе без отдельного решения о полномочиях.
...
## Что важно проверить при переносе существующих данных
...
8. В регламентированном процессе жёсткую привязку назначает предусмотренная команда; отдельное разрешение ручного назначения не предоставляется автоматически всем пользователям.
...
- [Временная проработка](Selection-temporary-preference.md)
- [Версии и состояния](Versions-and-composition.md)
