<!-- Generated from wiki/Selection-change-package.md. Edit the source, then run tools/Build-Book.ps1. -->

# Согласованный будущий состав: работа, контекст и пакет изменения

<a id="какую-потребность-предприятия-сохраняем"></a>

## Какую задачу предприятия решаем

Замена подшипника редуктора затрагивает вал, крышку, спецификацию, чертёж и технологическую операцию. Конструктору и технологу нужно видеть изменения совместно, пока производство продолжает работать по утверждённому основанию. Для этого предусмотрены контексты редактирования, в том числе связанные с извещениями.

В Interlink **рабочая область, совместный просмотр и выпуск имеют разные границы**. Пакет изменения описывает конкретные действия и не заменяет собой долговечный контекст подбора.
...
Это принятая целевая модель, а не готовое рабочее место. Сквозная реализация рабочих областей, контекстов, публикации и восстановления запланирована и должна быть испытана на пользовательских сценариях. Формы, задания и маршруты согласования создаёт прикладная система поверх Interlink.

## Связанные документы

- [Контексты подбора и совместная работа](Selection-contexts.md)
- [Мягкая конкретизация позиции](Selection-soft-concretion.md)
- [Последовательное проведение изменений](Selection-sequential-changes.md)
