<!-- Generated from wiki/Change-package-concept.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Рабочая область может пережить несколько проведений. Контекст может продолжать использоваться после окончания отдельных работ. Объединение контекстов для совместного просмотра не обязывает конструкторов и технологов одновременно выпускать все результаты. Подробности — в [рабочих областях и параллельной работе](Workspaces-and-parallel-work.md) и [контекстах подбора](Selection-contexts.md).
...
## Какие задачи решает разделение механизмов

Пользовательские предметные требования сохраняются: незавершённые правки нужно отделять от архивных данных, связанные работы — просматривать совместно, итерации — возвращать без потери истории, а изменение — вводить управляемо. Но одинаковая потребность не требует одинакового внутреннего контейнера.
...
Требования уточняются через обязательность извещения, разрешение параллельной работы и события автоматического пополнения контекста. Эти настройки проверяются на контрольных примерах и не превращаются в безусловные ограничения всех приложений Interlink.
