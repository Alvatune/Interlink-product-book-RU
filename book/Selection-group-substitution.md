<!-- Generated from wiki/Selection-group-substitution.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Interlink должен поддерживать замены вида N:M: один набор из N позиций заменяется другим набором из M позиций. Это **групповая замена**, а не семейство версий одной детали: меняется полный набор участников и их количеств.
...
Interlink должен поддерживать исходный, актуальный и допустимые варианты группы, а также соответствующие представления состава. Если процесс требует ровно одного актуального варианта авторской группы, это ограничение действует в её области. Оно не запрещает разным заказам выбирать разные разрешённые комплекты из общей библиотеки.
...
Interlink проверяет совокупное действие выбранных решений. Совместимые независимые замены могут применяться вместе; конфликтующие требуют переработки плана или явно поддержанной композиции. Ограничения повторного участия позиций задаются предметным процессом и не должны без объяснения запрещать общий участник, который лишь проверяется и не потребляется дважды.
...
- Какие данные, связи и представления нужны пользователю для групп, вариантов и участников замен?
- Может ли один участник входить сразу в две перекрывающиеся группы?
- Как должен разрешаться конфликт нескольких подходящих вариантов?
...
Библиотечные описания, привязка участников, независимый выбор контекста и совокупная проверка применения приняты как целевой контракт. Это ещё не готовая подсистема. Реализация и перенос должны пройти проверки полных N:M-комплектов, режимов состава, актуализации вариантов, общей и переменной части исполнений и подтверждённых ограничений повторного участия позиций.

## Связанные документы

- [Допустимые замены: от разрешения до применения](Allowed-replacements.md)
- [Глобальный аналог объекта](Selection-global-analog.md)
- [Матрицы совместимости](Compatibility-matrices.md)
- [Условия включения позиций и конфигуратор](Selection-position-presence.md)
