<!-- Generated from wiki/Digital-twins.md. Edit the source, then run tools/Build-Book.ps1. -->
...
## Какие требования необходимо выполнить и что добавляется

Экземпляры, партии, заказы, доработки и связанные документы входят в обязательную программу сценариев. Их нельзя отложить целиком под названием «будущий цифровой двойник». Результат проверяется на конкретных данных и процессах предприятия.

Направления развития включают явное разделение проекта, экземпляра и функционального места; два времени исторического чтения; точные основания внешних наблюдений и агентских выводов; самостоятельную историю физических преобразований и долгоживущих объектов. Для каждого направления необходимы собственные рабочие сценарии и критерии приёмки.

В строительстве та же основа связывает функциональную систему, помещение и сменяемое оборудование. В пищевом производстве — рецептуру, партии сырья, преобразование и испытание образца. В логистике — содержимое грузового места, переупаковку и движение. Измерение или перемещение груза не обязаны получать фиктивную инженерную версию. Подробнее — [сценарии разных отраслей](Multidomain-scenarios.md).
...
Первый полноценный сценарий предстоит выбрать с пользователями: отзыв партии, гарантийный сервис, контроль ремонта или сравнение проекта с фактической установкой. Это определит полезный объём внедрения, но не отменит уже обязательные сценарии.

Точные ссылки, история, экземпляры, партии, сервис и сохранность описаны как требования к целевому поведению и планы модулей. Наличие схемы или небольшого испытания не означает готового промышленного двойника. Alvatune предполагается отдельным приложением управления агентской работой; он может использовать эти данные, но не заменяет производственную или сервисную систему автоматически.

Связанные страницы: [экземпляры, партии и фактический состав заказа](Order-units-lots-as-built.md), [объектный фундамент](Object-foundation.md), [агенты и прослеживаемость](Agents-and-traceability.md), [производительность и агентская нагрузка](Performance-and-agents.md), [текущая готовность и план](Current-state-and-roadmap.md), [вопросы о дальнейшем развитии](Questions-agents.md).
