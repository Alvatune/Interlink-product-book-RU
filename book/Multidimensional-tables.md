<!-- Generated from wiki/Multidimensional-tables.md. Edit the source, then run tools/Build-Book.ps1. -->
...
## Что это добавляет относительно справочных сценариев

Обязательное покрытие инженерных справочников не сводится к полной сетке: многие существующие таблицы являются перечнями строк и должны такими остаться. Полная многомерная сетка — дополнительный явно выделенный договор для программ испытаний, числовых справочников и других задач, где требуется каждая заданная комбинация.

Полная сетка позволяет проверять наличие каждой предусмотренной точки и обслуживать адресный поиск без отдельных объектов на каждое число. Она применяется только там, где полнота декартова произведения осей действительно является требованием; обычный перечень существующих строк не следует принудительно превращать в такую сетку.
...
Смежные темы: [выбор формы табличных данных](Tabular-data.md), [обычные справочники](Reference-tables.md), [разреженные карты](Coordinate-maps.md), [правила совместимости](Compatibility-matrices.md).
