<!-- Generated from wiki/Order-production-documents.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Производственная ведомость, далее ПВ, не является другим названием заказа и не сводится к распечатке состава. Предусматривается самостоятельный версионируемый производственный документ со своей карточкой, номером, составом, прототипом, рабочими и опубликованными версиями, проверкой, согласованием и подписью.
...
Если производственная ведомость должна включать структуру и документы, для неё формируется самостоятельное содержание или доказательный комплект. Его нельзя отождествлять со снимком состава. Если ПВ относится к точной передаче, с её снимком связывается конкретная версия ПВ; в процессах, где ПВ готовит ПЗ или развивается параллельно, такая связь появляется позже либо не требуется. Снимок Interlink хранит позиции и точные версии, но не копирует все файлы, документы и атрибуты.
...
Отчёт для учётной системы, например 1С, должен строиться по составу заказа с учётом количества в ветви и количества всего заказа. Для пяти одинаковых шкафов система рассчитывает общую потребность в аппаратах, проводе и трудовых нормах.
...
Масса или длина провода может определяться по индексируемому справочнику. Паспорт отчёта сохраняет использованную редакцию строки, единицы и округление. Наличие в файле готового итога не заменяет происхождение расчёта. Подробнее — [справочные таблицы](Reference-tables.md).
...
- Какие регламентированные отчёты необходимо выпускать без изменения привычной формы?
...
Производственные ведомости и отчёты входят в обязательные целевые возможности. Приняты подробные контракты самостоятельной ПВ, копирования прототипа, точных оснований отчёта и публикации. Это ещё не готовое рабочее место: согласование, подпись, формы и полный производственный процесс требуют предметной реализации. Конкретный порядок ПВ и ПЗ, нумерация и обязательный комплект уточняются при обследовании предприятия.

## Связанные документы

- [Точный состав передачи](Order-exact-publication.md)
- [Маршруты и нормы](Order-routes-and-norms.md)
