<!-- Generated from wiki/Allowed-replacements.md. Edit the source, then run tools/Build-Book.ps1. -->
...
**Статус: принятый целевой контракт, реализация и пользовательские рабочие места планируются.** Обязательное покрытие включает одиночные и комплектные замены, ограничения по версиям и условиям применения, согласование и объяснение результата. Спорные режимы необходимо уточнить с владельцами процессов и проверить на приёмочных примерах.
...
Обязательный сценарий — замена нескольких исходных участников несколькими новыми. Например, двигатель и муфта заменяются мотор-редуктором, переходной плитой и комплектом крепежа. Разрешение относится к полному варианту. Нельзя взять мотор-редуктор из одного варианта, плиту из другого и назвать получившийся набор уже согласованным.
...
В одном процессе может требоваться ровно один действующий вариант авторской группы замен. В другом нужна библиотека нескольких разрешённых вариантов, из которых разные заказы выбирают независимо. Interlink должен поддерживать оба сценария: ограничение конкретной группы не становится общим ограничением библиотеки альтернатив.
...
Разрешение может относиться к объекту в целом, к определённой исходной инженерной версии или к её точному содержанию. Если допуск принадлежит версии детали, сначала требуется определить эту версию. Поэтому выбор заменителя не всегда предшествует определению исходной версии.
...
Эта последовательность отражает зависимость данных. Нельзя взять допуск соседней исходной версии, если нужный не найден, или рекурсивно перейти по цепочке аналогов без специального разрешённого правила.

После подбора новых участников проверяется [совместимость их фактического содержания](Compatibility-matrices.md). Например, монтажные отверстия конкретной плиты должны подходить к выбранному мотор-редуктору. Неудача этой проверки не разрешает системе молча взять следующую версию плиты. Допустим выход с новым объяснённым предложением, которое проходит собственное рассмотрение.
...
## Полнота сценариев, расширение и приёмка

Обязательное покрытие включает [глобальные аналоги](Selection-global-analog.md), [групповые замены](Selection-group-substitution.md), полные варианты, их представления и работу с вариантностью планового заказа. Разрешение одного актуального варианта в исходной группе сохраняется в соответствующем режиме совместимости, а не становится ограничением всей библиотеки Interlink.
...
Приёмка проверит независимый выбор двух заказов, полный отказ при неудаче участника комплекта, запрет скрытого снятия фиксации, версионного владельца аналога, сохранение общих предпосылок и отдельность фактической установки. Более сложный поиск оптимального комплекта остаётся направлением развития; обязательные сценарии не должны ждать универсального решателя.

Дальнейшее чтение: [совместимость](Compatibility-matrices.md), [табличные основания](Tabular-data.md), [изменения и свидетельства](Changes-and-evidence.md), [заказы](Orders-guide.md).
