<!-- Generated from wiki/Selection-position-presence.md. Edit the source, then run tools/Build-Book.ps1. -->

# Условия включения позиций и конфигуратор изделия

> Здесь описан целевой механизм. Полное покрытие сценариев конфигуратора и новые проверки совместимости должны быть подтверждены реализацией и приёмочными примерами.

## Почему этот сценарий связан с подбором версий

Формирование состава включает не только выбор версий деталей и сборочных единиц. Для конфигурируемого изделия сначала необходимо определить, какие позиции вообще входят в конкретное исполнение.
...
Interlink должен сохранить пользовательские возможности конфигуратора и явно разделяет эти решения. **Условие включения позиции** определяет, нужна ли строка состава. Подбор версии определяет используемое инженерное решение и его содержание. Проверка совместимости подтверждает допустимость всего выбранного комплекта; её нельзя заменить успехом подбора каждой детали по отдельности.

## Обязательные результаты сценария

Interlink должен поддержать следующие возможности:
...
Исключённая условием позиция не должна выглядеть как позиция, для которой не удалось подобрать версию. В первом случае строка не нужна данному исполнению; во втором состав остаётся неполным. Интерфейс и дальнейшие операции обязаны различать эти результаты.
...
| Аспект | Потребность пользователя | Решение Interlink |
|---|---|---|
...
- Какие виды опций должны войти в первую версию конфигуратора Interlink?
...
- Как должен обрабатываться один узел, повторно включённый в разные ветви с разными параметрами?
...
Разделение условий включения позиции, подбора версии и совместимости закреплено в целевой архитектуре Interlink. Также определён путь развития табличных правил и ограничений на несколько совместно выбранных участников. Это не утверждение о готовом конфигураторе: исполнение, интерфейс, вложенная передача параметров и полнота сценариев ещё должны пройти реализацию и приёмку. Расширенные проверки не отменяют обязательное покрытие обязательных сценариев.

## Связанные документы

- [Применяемость по головному изделию, серии и дате](Selection-effectivity.md)
- [Допустимые групповые замены](Selection-group-substitution.md)
- [Точный состав производственного заказа](Selection-order-snapshot.md)
