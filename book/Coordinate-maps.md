<!-- Generated from wiki/Coordinate-maps.md. Edit the source, then run tools/Build-Book.ps1. -->
...
**Статус: принятое направление и план реализации.** Ни описанные рабочие места, ни все виды поиска ещё не являются готовой подсистемой. Примеры ниже поясняют порядок работы с предметными данными, а не предписывают устройство интерфейса.
...
Для некоторых задач достаточно самого утверждения, что сочетание разрешено. Заводить фиктивную карточку «допустимая тройка» не требуется. Но такое утверждение уже имеет смысл [правила совместимости](Compatibility-matrices.md), а не просто найденного значения.
...
Такой предметный поиск входит в обязательное покрытие справочных сценариев. Отдельно Interlink планирует возможность явно использовать подтверждённое членство конкретного материала в группе. Например, точная марка входит в утверждённую группу алюминиевых сплавов, по которой задано сопоставление. В объяснении должны остаться использованные членства и редакции правил.

Для поиска по группам необходимо явно определить порядок ролей, симметрию пары, вложенность групп и приоритет пересекающихся правил. Если прямое сопоставление и два групповых основания дают разные результаты, процедура должна объяснить состав предложений или конфликт, а не скрыть его выбором первой строки.
...
Связанные материалы: [табличные формы](Tabular-data.md), [справочники](Reference-tables.md), [полные сетки](Multidimensional-tables.md), [допустимые замены](Allowed-replacements.md), [матрицы совместимости](Compatibility-matrices.md).
