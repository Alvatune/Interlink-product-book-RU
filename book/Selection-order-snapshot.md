<!-- Generated from wiki/Selection-order-snapshot.md. Edit the source, then run tools/Build-Book.ps1. -->

# Точный состав производственного заказа

> Описан планируемый порядок официальной передачи состава в производство. Его ещё предстоит реализовать и проверить целиком; готовые рабочие места заказа и публикации пока не поставлены.
...
Interlink должен сохранять точное дерево конкретной передачи заказа. Изменяемый заказ и неизменяемое свидетельство передачи имеют разные назначения и не подменяют друг друга.
...
Для длительного воспроизведения нужны не только ссылки на версии. Если передавались чертёж, программа обработки или нормы расхода, должны сохраняться требуемые байты, значения и способ их прочтения в границах соответствующего доказательного комплекта. Современный файл с тем же названием не заменяет старый. Сроки хранения задаются правилами предприятия и обязательными требованиями; при удалении по установленным правилам фиксируется, какие материалы выбыли и какие части комплекта больше нельзя воспроизвести.
...
- [Сквозное формирование многоуровневого состава](Selection-end-to-end.md)
