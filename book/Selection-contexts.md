<!-- Generated from wiki/Selection-contexts.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Interlink должен поддерживать обычные контексты, контексты извещений, их объединение и автоматическое пополнение. Их жизнь не связывается с временным контейнером проведения: завершение публикации не должно автоматически уничтожать нужный контекст чтения.

**Состояние реализации:** принятый целевой порядок работы на 6 сентября 2026 года. Полный цикл чтения, команд, переноса и пользовательских рабочих мест ещё предстоит проверить исполнением.
...
Однозначность назначения объекту в группе не запрещает разные версии одной детали на отдельных позициях. [Позиционная мягкая конкретизация](Selection-soft-concretion.md) имеет другую область; её взаимодействие с общим назначением определяется явным профилем.
...
Режим автоматического обновления — управляемая настройка. При его включении взятие версии на редактирование, создание новой инженерной версии и создание нового объекта в составе должны обновлять назначения обычного контекста по объявленным правилам. При выключении режима эти события сами список не меняют.
...
Конструкторский контекст использует станину Б с исправленным опубликованным содержанием. Технологический контекст закрепляет прежнее точное содержание той же Б, на котором рассчитаны базы обработки. При объединении нельзя считать их согласованными только по букве Б.
...
## Как проверять сценарий и что уточнить у специалиста
...
Далее: [мягкая конкретизация](Selection-soft-concretion.md), [пакет изменения](Selection-change-package.md), [последовательное проведение](Selection-sequential-changes.md), [итерации](Selection-iterations.md), [общий путеводитель](Version-selection-guide.md).
