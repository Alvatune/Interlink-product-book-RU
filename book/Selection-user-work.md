<!-- Generated from wiki/Selection-user-work.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Рабочая область, контекст и пакет не совпадают. Контекст отвечает за назначения, область — за незавершённое содержание, пакет — за совместное проведение. Их различия подробно описаны в [контекстах и группах](Selection-contexts.md).
...
| Профиль | Цель операции и разрешённая настройка | Производственный или явно выбранный редакторский режим |
...
Для каждого выбранного объекта проверяются жёсткие ограничения и строгая точность. При отсутствии жёсткого ответа профиль упорядочивает источники инженерной версии. В совместимом производственном профиле подбора: применяемость, контекст или группа, активная мягкая конкретизация, именованное правило, явно разрешённый резерв. В отдельном редакторском профиле контекст может предшествовать применяемости.
...
## Четыре примера работы

### 1. Конструктор готовит будущую серию редуктора

Серия 145 использует корпус Б; контекст опытной модернизации назначает В. В совместимом производственном профиле подбора применяемость сохраняет Б. Для проработки конструктор явно выбирает редакторский профиль и доступную рабочую область В.
...
Приёмка проверяет одинаковый ответ в дереве, отчёте и фоновом расчёте при одинаковых условиях; различие версии и содержания; сохранение soft и контекста после пакета; точность одобрения; полную совместимость; отсутствие скрытой подмены при ошибке.
...
Далее: [сквозные сценарии](Selection-end-to-end.md), [правила и их редакции](Selection-rule-lifecycle.md), [пакет изменения](Change-package-guide.md), [контексты](Selection-contexts.md), [мягкая конкретизация](Selection-soft-concretion.md), [снимок заказа](Selection-order-snapshot.md).
