<!-- Generated from wiki/Tabular-data.md. Edit the source, then run tools/Build-Book.ps1. -->
...
**Состояние описания: принятое направление и план реализации на 6 сентября 2026 года.** Ниже описан будущий пользовательский процесс, а не уже доступные рабочие места. Точные пределы размера, быстродействие и полнота переноса конкретной установки исходной системы должны быть подтверждены испытаниями.

<a id="три-формы--под-разные-обязанности-данных"></a>

## Три формы с разными требованиями к данным

| Форма | На какой вопрос отвечает | Что означает незаполненное место |
|---|---|---|
| [Справочник существующих строк](Reference-tables.md) | Какие типоразмеры, нормы или предложения действительно заданы? | Такой строки в выбранном источнике нет; все сочетания значений колонок не подразумеваются |
| [Полная многомерная таблица](Multidimensional-tables.md) | Есть ли результат для каждой точки объявленной конечной программы? | В рабочей таблице — незавершённость; в опубликованной полной сетке — нарушение заявленной полноты |
| [Карта сопоставлений](Coordinate-maps.md) | Какой результат задан для этого сочетания признаков? | Сопоставление не задано; запрет или разрешение требуют дополнительного правила |
...
Сначала определяют, что представляет набор данных и какие требования он должен выполнять: хранить отдельные строки, все сочетания конечных осей или только заданные сопоставления. После этого выбирают способ отображения. Изменение строк на столбцы не превращает справочник в полную сетку. Если таблица выражает ещё и инженерный допуск, к выбранной форме добавляются правила его проверки.
...
- [Допустимые замены](Allowed-replacements.md) объясняют, **что разрешено поставить вместо исходного объекта или целого комплекта**, в какой области и на каком основании.
- [Матрицы совместимости](Compatibility-matrices.md) объясняют, **могут ли все выбранные участники работать вместе**, включая ограничения конкретных версий, материалов, режима и общих ресурсов.

Например, справочник может вернуть три уплотнения, подходящих по размерам. Перечень заменителей может разрешать два из них для данного насоса. Проверка среды и температуры может оставить одно. Подбор версии этого уплотнения остаётся обычным [подбором версий](Version-selection-guide.md), а фактическая установка — отдельным [событием эксплуатации](Order-units-lots-as-built.md). Эти этапы нельзя заменить одним зелёным цветом клетки.
...
Приёмка должна показать не только красивую таблицу, но и устойчивые адреса после перестановки строк, управляемую загрузку без потери данных, различение неизвестности и запрета, историческое воспроизведение расчёта, а также корректное поведение при конфликте двух правок. Для переноса существующих данных дополнительно нужны реальные примеры источников, формул, правил подбора и спорных пустых клеток.

Стартовать можно с ближайшей задачи: [справочная таблица](Reference-tables.md), [полная многомерная сетка](Multidimensional-tables.md) или [разреженная карта](Coordinate-maps.md). Общая модель объясняется в [путеводителе по данным](Data-guide.md), состояние работ — в [дорожной карте](Current-state-and-roadmap.md).
