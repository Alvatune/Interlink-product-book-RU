<!-- Generated from wiki/Data-database-materialization.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Не все сведения нуждаются в инженерных версиях. Контактный номер хранится как реквизит карточки склада; его исправление проверяет, что карточка не была параллельно изменена другим участником. Измерение температуры можно добавлять как неизменяемый факт и исправлять новым фактом. Оба способа используют общие средства хранения, связей и контроля доступа. Для них не требуется создавать объект типа «Деталь» или вводить процесс инженерного извещения.
...
Названия и типы идентификаторов должны однозначно показывать, что адресуется: устойчивый объект, инженерная версия или её точное содержание. Нельзя использовать одно поле с разным смыслом в зависимости от экрана или неявного режима чтения.
...
Такое хранение создаёт основу для высокой параллельной нагрузки. Ожидаемый эффект проверяется на заданных моделях, запросах, объёмах, правах доступа и оборудовании, включая стоимость общего реестра, истории, индексов и конкурирующей записи. Индекс тоже имеет цену: он занимает место и замедляет часть изменений.
...
Справочник типоразмеров не обязательно превращать в тысячи самостоятельных инженерных деталей. В план добавлены [справочные таблицы](Reference-tables.md), [полные многомерные таблицы](Multidimensional-tables.md) и [координатные карты](Coordinate-maps.md). Их значения также должны иметь типизированное индексируемое хранение.
...
- заявленное быстродействие должно подтверждаться испытаниями, а не только схемой.
...
- [Производительность и агенты](Data-performance-and-agents.md)
