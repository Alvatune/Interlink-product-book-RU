<!-- Generated from wiki/Selection-global-analog.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Глобальные аналоги должны поддерживать период действия, приоритет и режим показа одного или нескольких кандидатов. Обязательное покрытие включает:
...
Этот пример также показывает границу глобального аналога. Если замена одного двигателя всегда требует добавить две другие позиции, одной связи «объект заменяет объект» может быть недостаточно. Тогда решение оформляется как [групповая замена](Selection-group-substitution.md).
...
Не следует превращать локальные замены в глобальные аналоги без отдельного разрешения. Это расширило бы область действия и могло бы предложить опасную замену в изделии, для которого она никогда не утверждалась.
...
- Какие условия действия глобальных аналогов необходимы предприятию?
...
- [Настраиваемое правило подбора версий](Selection-custom-rule.md)
