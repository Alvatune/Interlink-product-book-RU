<!-- Generated from wiki/Data-model-runtime.md. Edit the source, then run tools/Build-Book.ps1. -->
...
Например, пользователь исправляет контактный номер в существующей карточке склада; система проверяет, что карточку не изменил другой участник. Результат измерения записывается иначе: создаётся новый неизменяемый факт, связанный с измеряемым экземпляром. Для этих операций можно выбрать договоры записи без инженерных версий. Общими остаются проверка полномочий, защита от конкурирующей записи, история обязательных действий и понятный исход операции.
...
Целевые показатели должны подтверждаться нагрузочными испытаниями на данных и операциях промышленного масштаба. Архитектурное решение создаёт предпосылки, но не заменяет измерение.
...
Связанные документы: [жизненный цикл модели](Data-model-lifecycle.md), [физическая материализация](Data-database-materialization.md), [производительность и агенты](Data-performance-and-agents.md), [поиск и запросы](Data-search-and-queries.md).
